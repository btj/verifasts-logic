...
We say a heap $h$ and logical heap $H$ are \emph{consistent with depth bound $k$}, denoted $h \sim_k H$, if there exists a ghost heap $\hat{h}$, an atomic spaces bag $A$, and a consistent stock of lemma type chunks $\Sigma$ of size at most $k$ such that $h + \hat{h} + \mathsf{chunks}(A) + \Sigma \ge \mathsf{heaps}(A) + H$, where a heap is interpreted as a set of $\mapsto$ chunks and a ghost heap is interpreted as a set of $\gmapsto$ chunks. Notice: if $h \sim_k H$, it follows that $H$ is strongly consistent. We say $h$ and $H$ are \emph{consistent}, denoted $h \sim H$, if $\exists k.\;h\sim_k H$.
...
\begin{lemma}
$$\{P\}\ G\ \{Q\} \land H \in P \land h \sim_k H + H' \Rightarrow \exists H''.\;H'' \in Q \land h \sim_k H'' + H'$$
\end{lemma}
\begin{proof}
